...
struct MachOImportResult {
    bool ok = false;
    std::string error;
    std::vector<std::string> symbols;
};

namespace macho_import_detail {
...
inline bool rangeValid(std::size_t offset, std::size_t length, std::size_t size) {
...
inline MachOImportResult parseThin(const uint8_t* data,
                                   std::size_t size,
                                   std::size_t base,
                                   std::size_t sliceSize) {
    MachOImportResult out;
    ...
    if (commandsEnd < commandsStart || commandsEnd > base + sliceSize || commandsEnd > size) {
    ...
    if (nsyms > 1000000u) {
        out.error = "Mach-O symbol count exceeds parser limit";
        return out;
    }
    ...
    if (!rangeValid(symBase, symBytes, base + sliceSize) ||
        !rangeValid(strBase, strsize, base + sliceSize) ||
        !rangeValid(symBase, symBytes, size) ||
        !rangeValid(strBase, strsize, size)) {
        out.error = "Mach-O symbol or string table is truncated";
        return out;
    }

    for (uint32_t index = 0; index < nsyms; ++index) {
        const std::size_t entry = symBase + static_cast<std::size_t>(index) * 12u;
        const uint32_t stringOffset = read32(data + entry, little);
        const uint8_t type = data[entry + 4];
        ...
        if (!symbol.empty()) out.symbols.push_back(symbol);
    }

    std::sort(out.symbols.begin(), out.symbols.end());
    out.symbols.erase(std::unique(out.symbols.begin(), out.symbols.end()), out.symbols.end());
    out.ok = true;
    return out;
}
...
inline MachOImportResult parseArmv7MachOImports(const uint8_t* data, std::size_t size) {
    using namespace macho_import_detail;
    MachOImportResult out;
    if (!data || size < 4) {
    ...
    if (magicLE == kMHMagic || magicLE == kMHCigam) {
        return parseThin(data, size, 0, size);
    }
    ...
    for (uint32_t index = 0; index < count; ++index) {
        const std::size_t entry = 8 + static_cast<std::size_t>(index) * 20u;
        ...
        return parseThin(data, size, offset, sliceSize);
    }
