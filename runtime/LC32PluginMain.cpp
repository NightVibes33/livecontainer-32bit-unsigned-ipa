...
void logEvent(const std::string& stage,
    ...
    std::fprintf(stderr,
                 ...
                 stage.c_str(), pc, value, detail.c_str());
    ...
    std::fprintf(file,
                 ...
                 jsonEscape(stage).c_str(), jsonEscape(detail).c_str(), pc, value);
    std::fclose(file);
}
...
bool readFile(const char* path, std::vector<uint8_t>& bytes, std::string& error) {
    if (!path || !*path) { error = "path is empty"; return false; }
    std::ifstream input(path, std::ios::binary | std::ios::ate);
    if (!input) { error = std::string("open failed: ") + std::strerror(errno); return false; }
    ...
    if (!input.read(reinterpret_cast<char*>(bytes.data()), length)) {
        error = "read failed"; bytes.clear(); return false;
    }
    return true;
}
...
const char* LC32LastError(void) { return gLastError.c_str(); }
...
int LC32Main(int argc, char** argv) {
        ...
        for (const auto& dependency : metadata.dependencies) {
            ...
            if (virtualImage.kind != lc32::VirtualRuntimeImageKind::Unsupported) {
                ...
                for (const auto& capability : virtualImage.capabilities) {
                    logEvent("cleanroom-virtual-capability", capability);
                }
                continue;
            }
            logEvent(isWeak ? "cleanroom-weak-dependency" : "cleanroom-dependency", dependency.path);
            if (!isWeak && !firstUnsupported) firstUnsupported = &dependency;
        }
        if (firstUnsupported) {
    ...
    logEvent("guest-cpu-stop", stopDetail, result.cpuResult.pc, result.cpuResult.instruction);
    return 71;
}
