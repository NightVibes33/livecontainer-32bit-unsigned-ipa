...
void DyldBootSession::event(DyldBootResult& out,
                            std::string stage,
                            std::string detail,
                            uint32_t value) {
...
bool DyldBootSession::dispatchSupervisorCall(DyldBootResult& out,
                                              const Result& stop,
...
DyldBootResult DyldBootSession::executePrepared(DyldBootResult out,
    ...
    Memory memory{
        [this](uint32_t address, void* data, size_t size) {
            return read(address, data, size);
        },
        [this](uint32_t address, const void* data, size_t size) {
    ...
    out.cpuResult = {
        StopReason::StepLimit, state_.r[15], 0, maxSteps, "dyld boot step limit"};
    event(out, "dyld-step-limit");
    return out;
}

DyldBootResult DyldBootSession::boot(const DyldHandoffSpec& spec,
                                      uint64_t maxSteps) {
...
DyldBootResult DyldBootSession::bootImageSet(const DyldImageSetSpec& spec,
    ...
    event(out,
          ...
          "mapped " +
              std::to_string(out.imageSet.mappedDependencies.images.size()) +
              " dependency images",
...
DyldBootResult DyldBootSession::bootImpl(const DyldHandoffSpec& spec,
    ...
    if (auditContext && exists) {
        ...
        for (const std::string& rpath : out.dependencies.rpaths) {
            context.rpaths.push_back(rpath);
        }
        out.dependencyAudit = auditGuestDependencies(out.dependencies, context, *exists);
        event(out,
              out.dependencyAudit.ok ? "dependency-audit-passed"
