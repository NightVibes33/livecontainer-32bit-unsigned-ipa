...
CVReturn CVPixelBufferCreate(CFAllocatorRef a, size_t w, size_t h, OSType f, CFDictionaryRef d, CVPixelBufferRef *o) { if(!o || !w || !h) return kCVReturnInvalidArgument; *o=NULL; BOOL yuv=(f==0x34323076 || f==0x34323066); size_t bpp=(f==0x42475241 || f==0x41524742)?4:((f==0x4c303038 || yuv)?1:0); if(!bpp || w>SIZE_MAX/bpp || h>SIZE_MAX/(w*bpp)) return kCVReturnInvalidPixelFormat; LC32PixelBuffer *p=[LC32PixelBuffer new]; p->w=w; p->h=h; p->format=f; p->row=w*bpp; p->size=p->row*h; if(yuv) { size_t chromaHeight=(h+1)/2; if(chromaHeight>SIZE_MAX/p->row || p->size>SIZE_MAX-p->row*chromaHeight) {[p release];return kCVReturnInvalidSize;} p->planes=2; p->offset[0]=0; p->planeRow[0]=p->row; p->planeHeight[0]=h; p->offset[1]=p->size; p->planeRow[1]=p->row; p->planeHeight[1]=chromaHeight; p->size+=p->row*chromaHeight; } p->bytes=calloc(1,p->size); if(!p->bytes){[p release];return kCVReturnAllocationFailed;} p->owns=YES; *o=(CVPixelBufferRef)p; return kCVReturnSuccess; }
